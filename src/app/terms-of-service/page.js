...
export async function generateMetadata() {
  ...
  return {
    title: openGraphData?.data?.termsAndConditionsPage?.metaTitle || 'synaptix-labs',
    description: openGraphData?.data?.termsAndConditionsPage?.metaDescription || 'Welcome to Synaptix Labs - read expert blogs on AI, machine learning, and modern tech. Stay updated.',
    openGraph: {
      title: openGraphData?.data?.termsAndConditionsPage?.metaTitle || 'synaptix-labs',
      description: openGraphData?.data?.termsAndConditionsPage?.metaDescription || 'Welcome to Synaptix Labs - read expert blogs on AI, machine learning, and modern tech. Stay updated.',
...
export default async function Page() {
  try {
    const [termsAndConditionResponse,contactInfoResponse] = await Promise.all([
      fetch(`${process.env.NEXT_PUBLIC_API_URL}terms-and-condition`, { cache: 'no-store' }),
      fetch(`${process.env.NEXT_PUBLIC_API_URL}contact-info`, { cache: 'no-store' }),
    ]);


    const termsAndCondition = await termsAndConditionResponse.json();
